...
# w = np.arctan( np.diff( data[:,:,1], axis=1) / np.diff( data[:,:,0], axis=1) )
...
plt.subplot(1,2,1)
plt.hist( data_vw[:,:,0].flatten(), bins=np.linspace(0,1,20), density=True)
plt.xlabel('Speed |v| (mm/s)')

plt.subplot(1,2,2)
plt.hist( data_vw[:,:,1].flatten(), bins=20, density=True)
plt.xlabel('Ang. speed $\omega$ (1/s)')
...
draw_circle( [XY_SCALE*plate[0], XY_SCALE*plate[1]], XY_SCALE*1*plate[2] ,'r', lw=1, alpha=0.8)
...
tini, tfin = 0, n_frames/T_SCALE/60
dt = data_vw.shape[1]
...
for jj in range(17):
    ...
    plt.imshow( Indiv[jj,:,:].T, aspect='auto',  vmin=0, vmax=5, cmap='inferno_r')
    plt.xti
    plt.xlabel('|v| (mm/s)')
    plt.ylabel('$\omega$ (rad/s)')
plt.colorbar(label='Probability density')
plt.suptitle( DIR_NAME.split('/')[-1])
plt.tight_layout()
