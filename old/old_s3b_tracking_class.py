...
def progressive_scan(worms, blobs_t, MAX_STEP=100, N_STEP=10, SPEED_DUMP=0.5, VERBOSE=True):
    '''
    Parameters
    ----------
    worms : list
        List of Worm instances tracked until t-1.
    contours : list
        List of contours available at t.
    ...
    '''
    radii = np.linspace( 0 , MAX_STEP , N_STEP )    
    ...
    for _radius in radii:
        ...
        for worm_jj in worms_t_1_disp.copy():
            ...
            #... I compute the distance between its expected position to all blobs
    ...
    return worms, worms_t_1_disp, blobs_t_used




def naive_scan(worms, blobs_t, MAX_STEP=100, SPEED_DUMP=0.5, VERBOSE=True):
    for worm_jj in worms_t_1_disp:
        _worm_expected = WORMS[worm_jj].expected_position( alpha = SPEED_DUMP)
        dist = np.sqrt( np.sum( ( _worm_expected - blobs_t )**2, axis=1) )
        if np.any( dist < MAX_STEP ):
            #... link worm_jj to blob idx
            idx = np.argmin(dist)
            WORMS[ worm_jj ].update( blobs_t[idx] )
            if VERBOSE:
...
for t in tqdm(range(n_frames)):    
   ...
   #        the expected position of worms at time t
    ...
    # for _worm in worms:
        # worms_t_1.append( _worm.coordinates()[-1] )
        # worms_speed_t_1.append( _worm.speed()[-1] )
    ...
    ##########################
    # Progressive scan...
    WORMS, worms_t_1_disp, blobs_t_used = progressive_scan(WORMS, blobs_t) 

    # ... there will still be some un-tracked worms, waiting... in the dark...
    if VERBOSE:
