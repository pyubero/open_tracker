...
class Worm:
    def __init__(self, starting_position, t0=0, c=None, dtype='uint16'):
        self.t0= t0
        self.x = starting_position[0]*np.ones((2,), dtype=dtype)
        self.y = starting_position[1]*np.ones((2,), dtype=dtype)
        self.c = [c]
        self.dtype = dtype
        self.nresets = 0

    def soft_reset(self):
        self.t0 += len(self.x) - 2
        self.x = self.x[-2:]
        self.y = self.y[-2:]
        self.c = self.c[-2:]
    ...
    def expected_position(self, alpha=1, inertia=1):
        ...
        mean_speed = alpha * np.nanmean(self.speed()[-inertia:], axis=0)
        position = self.coordinates()[-1] + mean_speed
        return  position

    def update_contour(self, contour, autocenter=True):
        if (len(contour) > 0) and autocenter:
...
class MultiWormTracker:
    ...
    def export(self, filepath):
        with open(filepath, 'wb') as f:
            pickle.dump(self.WORMS, f)

    def export_matrix(self, filepath):
        np.savez(filepath, data=self.to_matrix())

    def to_matrix(self):
        n_worms  = len(self.WORMS)
        n_frames = self.N_CALLS + 2
        T0 = self.WORMS[0].t0
    ...
    def update(self, contours, VERBOSE = True):
        ...
        worms_t_1_disp = np.array(
            [ jj for jj, worm in enumerate(self.WORMS) if worm.x[-1] > 0]
        )
        ...
        for worm_jj in np.random.permutation(worms_t_1_disp):
            ...
            # t_without_moving = np.sum(abs(_subspeed) <= 1e-4)
            _worm_exists = [True for c in self.WORMS[worm_jj].c[-self.KEEP_ALIVE:] if len(c)==0]
            t_without_existing = np.sum(_worm_exists)

            
            #to_ignore = t_without_moving >= self.KEEP_ALIVE
            ...
            #if _valid_pos:
            #if to_ignore:
            if t_without_existing >= self.KEEP_ALIVE:
                expected_position = np.zeros((2,))*np.nan
                self.WORMS[worm_jj].update(expected_position)
                self.WORMS[worm_jj].update_contour([])

            else:
                self.WORMS[worm_jj].update(
                    self.WORMS[ worm_jj].expected_position(
                        alpha=self.SPEED_DAMP,
...
def generate_background(
    ...
    # Frame calculations
    initial_frame = int(videoCapture.get(1))
    total_frames = int(videoCapture.get(cv2.CAP_PROP_FRAME_COUNT) - 1)
    frames_idx = np.linspace(initial_frame, total_frames, n_imgs).astype("int")
    frames_idx = np.unique(frames_idx)

    # Creamos el modelo del fondo
    height = int(videoCapture.get(cv2.CAP_PROP_FRAME_HEIGHT))
    width = int(videoCapture.get(cv2.CAP_PROP_FRAME_WIDTH))
    fondo_ = np.zeros((height, width, n_imgs), dtype='uint8')


    # Loop to retrieve all necessary frames
    for ii, idx in tqdm(enumerate(frames_idx), total=len(frames_idx)):

        # Go to frame number...
        assert videoCapture.set(1, int(idx))
