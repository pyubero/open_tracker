...
while ret:
    ...
    _, gray = cv2.threshold( gray, 5, 255, cv2.THRESH_BINARY )
    ...
    output = cv2.drawContours( output, filtered_contours, -1, (0,255,0), 1, cv2.LINE_AA )
    _= [cv2.putText(gray, 
                    "%d" % cv2.contourArea(c),
                    (int(cv2.moments(c)["m10"] / cv2.moments(c)["m00"]),int(cv2.moments(c)["m01"] / cv2.moments(c)["m00"])),
                    font, fontsize, color, thickness, cv2.LINE_AA ) for c in cnt if  cv2.moments(c)["m00"]>0 ]
    cv2.circle( output,(plate[0], plate[1]), plate[2],(0,255,0),1)

    output = zoom_in( output, [0.5, 0.5], 3)
