...
BKGD_FILENAME   = os.path.join( OUTPUT_DIR, 'video_fondo.png')
...
BG_SKIP     =  34      #... number of discarded frames during background creation
MIN_AREA    = 40       #... minimum contour area, helps ignore salt and pepper noise
...
# Step -1. If DIR_PATH contains many videos with the same tag.
if os.path.isdir(DIR_PATH):
    VIDEO_LIST = [_ for _ in os.listdir(DIR_PATH) if _.split('.')[-1]=='avi' ]
    VIDEO_LIST.sort()
    print('Found %d videos in %s.' % (len(VIDEO_LIST), DIR_PATH) )
...
if GENERATE_BKGD:
    ...
    print('Video has %d frames.' % int(video.get(cv2.CAP_PROP_FRAME_COUNT)) )
...
print('Scale is: %1.2f px/mm' % (plate[2]/TRUE_PLATE_SIZE) )
...
mask_plate = cv2.circle(mask_plate, (plate[0],plate[1]), int(plate[2]), (255,), -1)
...
for VIDEO_FILENAME in VIDEO_LIST:
    ...
    for _ in tqdm(range(int(n_frames))):
        ...
        if curr_frame > MAX_FRAMES:
            break
        elif curr_frame<SKIP_INI_FRAMES:
            continue
